...
from schemas.geo_schemas import QueryResponse, QueryRequest
...
async def get_history(db: AsyncSession = Depends(get_async_session))-> list[QueryResponse]:
    ...
    try:
        ...
        queries = await db.execute(Geo.__table__.select())
...
@router.get("/history/{cadastre_number}", response_model=QueryResponse)
